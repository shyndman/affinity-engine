...
public class Group extends Positioned implements Iterable<Positioned> {
	...
	/**
	 * Returns the node at the specified position in this group. Returns {@code null} if the index is out of range (
	...
	/**
	 * Returns {@code true} if this Group contains the specified node.
	 */
	...
	/**
	 * Returns {@code true} if this Group is an ancestor of the specified node.
	 */
	...
	public void add(Positioned sprite) {
		...
		getContentNode().attachChild(sprite.getTopNode());
	}
	...
	public void add(int index, Positioned sprite) {
		...
		getContentNode().attachChildAt(sprite.getTopNode(), index);
	}
	...
	public void remove(Positioned sprite) {
		...
		if (index != -1) {
			...
			getContentNode().detachChildAt(index);
		}
	}
	...
	public void removeAll() {
		...
		children.clear();
		getContentNode().detachAllChildren();
	}
	...
	private void move(Positioned sprite, int position, boolean relative) {
		...
		if (oldPosition != -1) {
			...
			if (oldPosition != position) {
				...
				getContentNode().swapChildren(position, oldPosition);
			}
		}
	}
